"""
Service for retrieving supplier management filters.

This service handles the business logic for fetching and populating
filter options for the supplier-management module.
...
from apps.v1.api.api_filters.models.method import SupplierFilterDatabaseMethods
...
class SupplierFilterService:
    """
    Service class for retrieving supplier management filters.
    ...
    async def get_supplier_filters(
        ...
        """
        Retrieve filters for supplier-management module with populated options.

        Args:
            db: Database session
            module: Module name (should be 'supplier-management')

        Returns:
            StandardResponse with filter configuration and options
        """
        try:
            logger.info("STEP 1: Starting supplier filter retrieval workflow")
            ...
            db_methods = SupplierFilterDatabaseMethods(db)
            ...
            for filter_name, filter_def in filter_config.items():
                ...
                if filter_type == FILTER_TYPES["STATIC"]:
                    ...
                    logger.info(f"STEP 5.{filter_name}: Date range configured: {date_range_data}")
            ...
            logger.info("STEP 9: Supplier filters retrieved successfully")
            ...
            logger.error(
                f"STEP ERROR: Validation error in get_supplier_filters: {exc}",
                exc_info=True,
            )
            return StandardResponse(
                status=constant_variable.STATUS_FAIL,
            ...
            logger.error(
                f"STEP ERROR: Unexpected error in get_supplier_filters: {exc}",
                exc_info=True,
            )
            return StandardResponse(
                status=constant_variable.STATUS_FAIL,
    ...
    async def _get_dynamic_options(
        self,
        db_methods: SupplierFilterDatabaseMethods,
        source: str,
    ) -> list:
        """
        ...
        Args:
            ...
            source: Source identifier (country, category, sub_category, child_category)

        Returns:
            List of option dictionaries with label and value
        """
        source_method_map = {
            "country": db_methods.get_distinct_countries,
