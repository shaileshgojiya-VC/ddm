"""
Unified login service for authentication (handles both admin and user login).
"""
...
class LoginService:
    """
    Unified login service for handling both admin and user authentication.
    ...
    async def login(
        ...
        """
        Unified login service method that handles both admin and user login.
        ...
        """
        try:
            logger.info("STEP 1: Starting unified login workflow")
            ...
            logger.info("STEP 5: Verifying user role")
            ...
            # Determine if user is admin (case-insensitive check)
            is_admin = user.role.name.lower() == "admin"
            logger.info(
                f"STEP 5.5: User role verified - {'Admin' if is_admin else 'User'} login for: {user.email}"
            )
            ...
            logger.info(
                f"STEP 10: Login successful for {'admin' if is_admin else 'user'}: {user.email}"
            )
            ...
            logger.error(
                f"Validation error in login_service: {exc}",
                exc_info=True,
            )
            await db.rollback()
            return StandardResponse(
            ...
            logger.error(
                f"Database error in login_service: {exc}",
                exc_info=True,
            )
            await db.rollback()
            return StandardResponse(
            ...
            logger.error(
                f"Unexpected error in login_service: {exc}",
                exc_info=True,
            )
            await db.rollback()
            return StandardResponse(
