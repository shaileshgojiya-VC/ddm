"""
Admin login service for authentication.
"""
...
from apps.v1.api.auth.schema import AdminLoginRequest
from apps.v1.api.auth.serializer import AdminLoginSerializer
...
class AdminLoginService:
    """
    Admin login service for handling admin authentication.
    """
    ...
    async def admin_login(
        ...
        Args:
            db: Database session
            body: Admin login request containing email and password

        Returns:
            StandardResponse with login result
        """
        try:
            logger.info("STEP 1: Starting admin login workflow")
            ...
            logger.info("STEP 5: Verifying admin role")
            ...
            # Verify user has admin role (case-insensitive check)
            admin_role_name = "admin"
            if user.role.name.lower() != admin_role_name:
                logger.info(
                    f"User {user.email} with role {user.role.name} is not authorized for admin login"
                )
                return StandardResponse(
                    status=constant_variable.STATUS_FAIL,
                    status_code=status.HTTP_403_FORBIDDEN,
            ...
            logger.info(f"STEP 5.5: Admin role verified for user: {user.email}")
            ...
            # Serialize login response data using AdminLoginSerializer
            try:
                user_serializer = AdminLoginSerializer()
            ...
            logger.info("STEP 10: Admin login successful")
            ...
            logger.error(
                f"Validation error in admin_login_service: {exc}",
                exc_info=True,
            )
            await db.rollback()
            return StandardResponse(
            ...
            logger.error(
                f"Database error in admin_login_service: {exc}",
                exc_info=True,
            )
            await db.rollback()
            return StandardResponse(
            ...
            logger.error(
                f"Unexpected error in admin_login_service: {exc}",
                exc_info=True,
            )
            await db.rollback()
            return StandardResponse(
